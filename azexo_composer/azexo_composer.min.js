...
e(t).val('<div class="az-element az-container" data-az-type="textarea" data-az-name="'+a+'">'+i+"</div>")}})}})},e(document).on("azexo_add_element",n.save_container),e(document).on("azexo_edited_element",n.save_container),e(document).on("azexo_update_element",n.save_container),e(document).on("azexo_delete_element",n.save_container),e(document).on("azexo_update_sorting",n.save_container)}})},show:function(){this.each(function(){})},hide:function(){this.each(function(){var t=e(this).data("azexo_composer")
...
t.in_timeout=setTimeout(function(){if(""==t.attrs.an_letters)if("js"==t.attrs.an_in){e(t.dom_element).css("opacity","")
...
0==e(t.dom_element).parents(".azexo-animations-disabled").length&&(""!=t.attrs.an_in||""!=t.attrs.an_js_in)&&(t.out_timeout>0?t.animated?(t.clear_animation(),t.set_in_timeout()):(clearTimeout(t.out_timeout),t.hidden_after_in||t.set_in_timeout()):t.set_in_timeout())},set_out_timeout:function(){var t=this
t.out_timeout=setTimeout(function(){if(""==t.attrs.an_letters)if("js"==t.attrs.an_out){e(t.dom_element).css("opacity","")
...
for(var n=0;n<this.params.length;n++)("an_js_in"==this.params[n].param_name||"an_js_out"==this.params[n].param_name)&&(this.params[n].value=e)},edit:function(){this.update_js_animations_list(),st.baseclass.prototype.edit.apply(this,arguments)},show_controls:function(){var t=this
...
t.attrs.x=""+p.data.x,t.attrs.y=""+p.data.y,t.attrs.rotate_z=""+p.data.rotate,t.attrs.scale=""+p.data.scale}function i(){p.data.x=parseFloat(p.$node[0].dataset.x)||m.x,p.data.y=parseFloat(p.$node[0].dataset.y)||m.y,p.data.scale=parseFloat(p.$node[0].dataset.scale)||m.scale,p.data.rotate=parseFloat(p.$node[0].dataset.rotate)||m.rotate,a()}function o(){clearTimeout(f),f=setTimeout(function(){p.$node[0].dataset.scale=p.data.scale,p.$node[0].dataset.rotate=p.data.rotate,p.$node[0].dataset.x=p.data.x,p.$node[0].dataset.y=p.data.y,d.impress.initStep(p.$node[0]),a(),l(p.$node)},20)}function r(e){e.preventDefault(),e.stopPropagation()
...
var u=null,f=null
"step_controls"in d&&d.step_controls.remove(),d.step_controls=e('<div class="az-step-controls '+n+'btn-group-vertical"></div>').hide(),e('<button title="'+c("Move this step")+'" class="control '+n+"btn "+n+"btn "+n+"glyphicon "+n+'glyphicon-move"> </button>').data("func","move").appendTo(d.step_controls),e('<button title="'+c("Scale this step")+'" class="control '+n+"btn "+n+"btn "+n+"glyphicon "+n+'glyphicon-resize-full" > </button>').data("func","scale").appendTo(d.step_controls),e('<button title="'+c("Rotate this step")+'" class="control '+n+"btn "+n+"btn "+n+"glyphicon "+n+'glyphicon-refresh" > </button>').data("func","rotate").appendTo(d.step_controls)
var u
e(d.step_controls).appendTo(d.dom_element).on("mousedown","button",function(t){return t.preventDefault(),h.activeFunction=_[e(this).data("func")],i(),h.prevX=t.pageX,h.prevY=t.pageY,e(document).on("mousemove.az_presentation",r),!1}).on("mouseenter",function(){clearTimeout(u)}),e(document).on("mouseup",function(){h.activeFunction=!1,e(document).off("mousemove.az_presentation")}),e(d.dom_element).on("mouseenter",".step",function(){var t=e(this)
u=setTimeout(function(){h.activeFunction||(p.$node=t,l(p.$node))},500),t.data("showTimer",u)}).on("mouseleave",".step",function(){clearTimeout(e(this).data("showTimer"))})
var g=!1,v=0,y="0s"
e(document).off("scroll.az_presentation"),e(document).on("scroll.az_presentation",function(e){return g&&window.scrollTo(0,v),e.preventDefault(),e.stopPropagation(),!1}),e(d.dom_element).on("mouseenter",function(){v=e(document).scrollTop(),y=e(d.dom_content_element).css("transition-duration"),e(d.dom_content_element).css("transition-duration","0s"),g=!0}),e(d.dom_element).on("mouseleave",function(){e(d.dom_content_element).css("transition-duration",y),g=!1}),e(d.dom_element).on("mousewheel",function(t){var n=.05,a=t.originalEvent.wheelDelta/120,i=e(d.dom_content_element).css("transform"),o=1+a*n
e(d.dom_content_element).css("transform",i+" scale("+o+")")})}},update_sortable:function(){if(window.azexo_editor);},click_add_step:function(e){return e.data.object.add_step(),!1},add_step:function(){var e=new bt(this,!0)
...
e('<li><a href="#" class="'+t+'next">&raquo;</a></li>').appendTo(n)}yt.baseclass.prototype.render.apply(this,arguments)}}),Z("az_step",!0,bt),R(bt.prototype,{name:p("Step"),params:[W({type:"integer_slider",heading:p("Width"),param_name:"width",min:"1",max:"10000",value:"900"}),W({type:"integer_slider",heading:p("Height"),param_name:"height",min:"1",max:"10000",value:"700"}),W({type:"integer_slider",heading:p("X coordinate"),param_name:"x",min:"0",max:"10000",value:"0"}),W({type:"integer_slider",heading:p("Y coordinate"),param_name:"y",min:"0",max:"10000",value:"0"}),W({type:"integer_slider",heading:p("Z coordinate"),param_name:"z",min:"0",max:"10000",value:"0"}),W({type:"integer_slider",heading:p("X rotate"),param_name:"rotate_x",min:"-180",max:"180",value:"0"}),W({type:"integer_slider",heading:p("Y rotate"),param_name:"rotate_y",min:"-180",max:"180",value:"0"}),W({type:"integer_slider",heading:p("Z rotate"),param_name:"rotate_z",min:"-180",max:"180",value:"0"}),W({type:"integer_slider",heading:p("Scale"),param_name:"scale",min:"0",max:"10",value:"1",step:"0.01"})].concat(bt.prototype.params),hidden:!0,is_container:!0,show_parent_controls:!0,disallowed_elements:["az_presentation"],get_empty:function(){return'<div class="az-empty"><div class="top-left '+n+'well"><h1>↖</h1>'+p("Settings for this presentation element and for current step. ")+'<span class="'+n+"glyphicon "+n+'glyphicon-plus-sign"></span>'+p(" - add a new step.")+"</div></div>"},get_my_shortcode:function(){return this.get_children_shortcode()},show_controls:function(){bt.baseclass.prototype.show_controls.apply(this,arguments),window.azexo_editor&&(e(this.controls).find(".drag-and-drop").remove(),e('<span class="control '+n+"btn "+n+"btn-primary "+n+'glyphicon">'+this.name+"</span>").prependTo(this.controls))},update_sortable:function(){if(window.azexo_editor){var t=this
