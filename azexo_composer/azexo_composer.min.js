...
var Pt=(""+arguments.callee).replace(/((\/\/.*$)|(\/\*[\s\S]*?\*\/)|(\s))/gm,"").match(/^function\s*[^\(]*\(\s*([^\)]*)\)/m)[1].split(/,/),Dt=Pt[0],At=Pt[1],Rt=Pt[2],Lt=Pt[3],Ut=Pt[4],Et=Pt[5],Mt=Pt[6],Bt=Pt[7],Nt=Pt[8],Yt=Pt[9]
...
t.length>0&&this.render_form(t)},render:function(t){this.form_value=t,this.dom_element=e('<div class="'+n+'form-group"></div>')},opened:function(){"instance"in this&&this.render_form(this.instance)}}),"azexo_param_types"in window)for(var Ft=0;Ft<window.azexo_param_types.length;Ft++){var Ht=window.azexo_param_types[Ft],Xt=function(){Xt.baseclass.apply(this,arguments)}
X(Ht.type,Xt),Ht.baseclass=Xt.baseclass,A(Xt.prototype,Ht)}if(window.azexo_add_css=function(t,n){var a=window.azexo_baseurl+t
...
this.parse_shortcode('[az_row][az_column width="1/1"][az_text]'+t+"[/az_text][/az_column][/az_row]")}_.each(i,function(e){var a=e.match($t(n)),i=a[5],o=RegExp("^[\\s]*\\[\\[?("+_.keys(K.prototype.tags).join("|")+")(?![\\w-])"),s=qt.shortcode.attrs(a[3]),r=a[2]
...
b[e].timeline.push({tweens:[]}),b[n]=b[e].timeline[b[e].timeline.length-1],w[n]="step",x[n]=e,v(n)}function f(t){function a(t,a,i){function o(t,o,l){function d(){var t=e(h).find('[name="'+p+'"]').val()
""!=o&&o!=t&&delete a[i][o]
...
l=_.object(l,l),e(d).find(".options").append('<div class="'+n+'form-group"><label>'+p("Ease")+"</label><div>"+$(l,"ease",b[t].ease)+'</div><p class="'+n+'help-block">'+p("Ease")+"</p></div>"),e(d).find('.options [name="ease"]').on("change",function(){b[t].ease=e(this).val()}),e(d).find('.options [name="ease"]').chosen({search_contains:!0}),e(d).find(".options").append('<div class="'+n+'form-group delay"><label>'+p("Delay")+'</label><div><input class="'+n+'form-control" name="delay" type="text" value="'+b[t].delay+'"></div><p class="'+n+'help-block">'+p("Amount of delay in seconds before the tween should begin.")+"</p></div>"),e(d).find('.options [name="delay"]').on("change",function(){b[t].delay=e(this).val()}),"0"!=b[i].duration&&"-1"!=b[i].duration&&(e(d).find(".options ."+n+"form-group.duration").css("display","none"),e(d).find(".options ."+n+"form-group.delay").css("display","none"))}function v(e){b[e].tweens.push({target:"",duration:1,from:{},to:{},ease:"Linear.easeNone",delay:"0"})
var t=k.jstree("create_node",e,{type:"tween",text:o(b[e].tweens[b[e].tweens.length-1]),state:{opened:!0}},"last")
b[t]=b[e].tweens[b[e].tweens.length-1],w[t]="tween",x[t]=e}function g(e){switch(w[e]){case"scene":var n=t.an_scenes.indexOf(b[e])
if(n>-1)return[n]
break
...
v(e[0])}),e('<button title="'+c("Delete")+'" class="delete '+n+"btn "+n+'btn-default">'+p("Delete")+"</button>").appendTo(z).click(function(){var e=k.jstree("get_selected")
...
break
default:e(z).find(".add-step").attr("disabled","disabled"),e(z).find(".add-tween").attr("disabled","disabled")}}}).on("delete_node.jstree",function(e,n){var a=(n.node.id,g(n.node.id))
switch(a.length){case 1:t.an_scenes.splice(a[0],1)
break
case 2:t.an_scenes[a[0]].timeline.splice(a[1],1)
break
case 3:t.an_scenes[a[0]].timeline[a[1]].tweens.splice(a[2],1)}}).on("move_node.jstree",function(e,n){var a=g(n.node.id)
...
break
case 3:t.an_scenes[a[0]].timeline[a[1]].tweens.splice(n.position,0,t.an_scenes[a[0]].timeline[a[1]].tweens.splice(n.old_position,1)[0])}else{var i=g(n.parent),o=null
