...
var h=st.prototype.param_types.icon.prototype.icons.map(function(e){return e.replace(/^/,".").replace(/ /,".")}),u=h.join(", ")
for(var f in t){var v=t[f].name,g=t[f].html,y=f.split("|")
y.pop(),y=y.join("/")
var b=window.azexo_baseurl+"../azexo_elements/"+y+"/"
"baseurl"in t[f]&&(b=t[f].baseurl)
var w=""
"thumbnail"in t[f]&&(w=t[f].thumbnail)
var x=g.indexOf("az-rootable")>=0,z=function(){function t(t){function o(e){return 0==e.indexOf("azexo_elements")?window.azexo_baseurl+"../"+e:0!=e.indexOf("/")&&0!=e.indexOf("http://")&&0!=e.indexOf("https://")?n.baseurl+e:e}for(var i in a){var s=a[i]
...
y=H(y),y=y.replace(/\-value\: /g,": "),y=y.replace("border-top-color","border-color"),y=y.replace("border-top-left-radius","border-radius"),y=y.replace("border-top-style","border-style"),y=y.replace("background-position-x: 50%; background-position-y: 50%;","background-position: center;"),y=y.replace("background-position-x: 50%; background-position-y: 100%;","background-position: center bottom;"),y=y.replace("background-repeat-x: no-repeat; background-repeat-y: no-repeat;","background-repeat: no-repeat;"),y=y.replace("background-repeat-x: repeat;","background-repeat: repeat-x;"),st.prototype.show_editor(s,{name:p("Content"),attrs:{content:_,link:c,image:r,el_class:g,style:y,icon:m}},function(s){n&&(""!=m&&(e(t).removeClass(m),s.el_class=s.el_class+" "+s.icon),"A"==e(t).prop("tagName")&&e(t).attr("href",s.link),"IMG"==e(t).prop("tagName")?e(t).attr("src",s.image):""!=_&&""!=s.content&&e(t).html(s.content)),a&&(e(t).attr("class",s.el_class),e(t).attr("style",s.style)),d.attrs.content=e(d.dom_content_element).html(),d.restore_content(),o(),l(),i()})}function a(t){var n=e(t).clone()
...
var m=[],h=null,u=null
e(d.dom_element).off("mousemove.az-able").on("mousemove.az-able",function(){null!=h&&e(h).hasClass("sortable-highlight")&&(clearTimeout(u),u=setTimeout(function(){t(h)},1e3))})
...
e(d.dom_element).find(".az-sortable-controls").remove(),e(d.dom_element).find(".sortable-highlight").removeClass("sortable-highlight"),null!==h&&clearTimeout(u),e(n).addClass("sortable-highlight"),m.push(n),h=n,u=setTimeout(function(){t(n)},1e3)}}),e(d.dom_element).find(d.sortable[n]).find("> *").off("mouseleave.az-sortable").on("mouseleave.az-sortable",function(){if(0==e(this).closest("[data-az-restore]").length){var n=this
e(d.dom_element).find(".az-sortable-controls").remove(),e(d.dom_element).find(".sortable-highlight").removeClass("sortable-highlight"),null!==h&&clearTimeout(u),m.pop(),m.length>0?(n=m[m.length-1],e(n).addClass("sortable-highlight"),h=n,u=setTimeout(function(){t(n)},1e3)):h=null}})}(a)
